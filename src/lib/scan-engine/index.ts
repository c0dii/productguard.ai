...
export async function scanProduct(scanId: string, product: Product): Promise<void> {
  ...
  try {
    ...
    logger = new ScanLogger(scanId, product.id, product.user_id, product, {
      ...
      aiConfidenceThreshold: parseFloat(process.env.AI_CONFIDENCE_THRESHOLD || '0.40'),
      runNumber,
    });
    ...
    const aiConfidenceThreshold = parseFloat(process.env.AI_CONFIDENCE_THRESHOLD || '0.40');
