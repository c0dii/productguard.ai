...
export function PendingVerificationList({
  ...
  // Inline quick actions (confirm/dismiss buttons in the list row)
  ...
  // Drawer action handler — removes item from list and auto-advances to the next
  const handleDrawerAction = useCallback((infringementId: string) => {
    setInfringements((prev) => {
      const currentIndex = prev.findIndex((inf) => inf.id === infringementId);
      const filtered = prev.filter((inf) => inf.id !== infringementId);

      // Auto-advance: select the next item in the list (or previous if at end)
      if (filtered.length > 0) {
        const nextIndex = Math.min(currentIndex, filtered.length - 1);
        setSelectedInfringement(filtered[nextIndex] ?? null);
      } else {
        // No more items — close drawer
  ...
  return (
    <>
      <Card>
        ...
        <div className={`space-y-3 ${loading ? 'opacity-50 pointer-events-none' : ''}`}>
          {infringements.map((infringement) => (
            <div
              key={infringement.id}
              onClick={() => setSelectedInfringement(infringement)}
              className={`p-3 sm:p-4 rounded-lg bg-pg-bg border transition-all cursor-pointer ${
                selectedInfringement?.id === infringement.id
      ...
      <InfringementReviewDrawer
        ...
        onClose={() => setSelectedInfringement(null)}
